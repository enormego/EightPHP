<?php
/**
 * Contains examples of various Eight library examples. You can access these
 * samples in your own installation of Eight by going to ROOT_URL/examples.
...
class Controller_Examples extends Controller {
	...
	//		$archive->add(APPPATH.'views/', 'app_views/', YES);
	...
	function session() {
		...
		echo '<pre>'.print_r($_SESSION, YES)."</pre>\n";
	 ...
	 * Table Structure:
	 *  CREATE TABLE `pages` (
	 *  `id` mediumint( 9 ) NOT nil AUTO_INCREMENT ,
	 *  `page_name` varchar( 100 ) NOT nil ,
	 *  `title` varchar( 255 ) NOT nil ,
	 *  `content` longtext NOT nil ,
	 *  `menu` tinyint( 1 ) NOT nil default '0',
	 *  `filename` varchar( 255 ) NOT nil ,
	 *  `order` mediumint( 9 ) NOT nil ,
	 *  `date` int( 11 ) NOT nil ,
	 *  `child_of` mediumint( 9 ) NOT nil default '0',
	 *  PRIMARY KEY ( `id` ) ,
	...
	*/
	function database() {
		...
		if($db->table_exists($table)) {
			...
			echo 'Table Listing:<pre>'.print_r($db->list_tables(), YES).'</pre>';
			...
			echo 'First:<pre>'.print_r($query->current(), YES).'</pre><br />';
			$query->next();
			echo 'Second:<pre>'.print_r($query->current(), YES).'</pre><br />';
			$query->next();
			echo 'Third:<pre>'.print_r($query->current(), YES).'</pre>';
			echo '<h3>And we can reset it to the beginning:</h3>';
			$query->rewind();
			echo 'Rewound:<pre>'.print_r($query->current(), YES).'</pre>';
	...
	/*function payment() {
		...
		echo '<pre>'.print_r($credit_card, YES).'</pre>';

		echo 'Success? ';
		echo ($response = $credit_card->process() == YES) ? 'YES!' : $response;
	}*/
