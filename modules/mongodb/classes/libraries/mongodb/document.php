 ...
 * <code>
 * class Model_Post extends MongoDB_Document {
 *   protected $name = 'posts';
 *   // All model-related code here
 * }
 * $post = MongoDB_Document::factory('post', $post_id);
 * </code>
 *
 * 2. Row Data Gateway pattern:
 * <code>
 * class Model_Post_Collection extends MongoDB_Collection {
 *   protected $name = 'posts';
 ...
 *
 * <code>
 *   class Model_Document extends MongoDB_Document {
 *     public $name = 'test';
 *   }
 *   $document = new Model_Document(); // or MongoDB_Document::factory('document');
 *   $document->name = 'Mongo';
 *   $document->type = 'db';
 *   $document->save();
 ...
 *   }
 *
 *   class Model_User extends MongoDB_Document {
 *     protected $name = 'users';
 *   }
 *
 *   $user = MongoDB_Document::factory('user')->set('id','colin')->set('email','colin@mollenhour.com');
 *   $post = MongoDB_Document::factory('post');
 *   $post->user = $user;
 *   $post->title = 'MongoDb';
 *   $post->save()
...
abstract class MongoDB_Document {
    ...
    /**
     * Instantiate an object conforming to MongoDB_Document conventions.
     ...
     *
     *  If using a corresponding MongoDB_Collection subclass, set this only in the MongoDB_Collection subclass.
     *
     *  @var  string
     */
    protected $name;
  ...
  /** A cache of MongoDB_Collection instances for performance
   *  @static  array */
  ...
  public function collection($fresh = FALSE)
  {
    if($fresh === TRUE)
    {
      if($this->name)
      {
        return new MongoDB_Collection($this->name, $this->db, $this->gridFS, get_class($this));
      }
      else
      {
    ...
    if($this->name)
    {
      ...
      {
        self::$collections[$name] = new MongoDB_Collection($this->name, $this->db, $this->gridFS, get_class($this));
      }
  ...
  public function __call($name, $arguments)
  {
    ...
    switch($parts[0])
    {
      case 'find':
        ...
        return MongoDB_Document::factory($this->_searches[$search]['model'])
                ->collection(TRUE)
  ...
  public function __get($name)
  {
    ...
    if(array_key_exists($name, $this->_references))
    {
      if( ! isset($this->_related_objects[$name]))
      {
        ...
        if ($foreign_field) {
          $this->_related_objects[$name] = MongoDB_Document::factory($model)
            ->collection(TRUE)
        ...
        else
        {
          ...
          $this->_related_objects[$name] = MongoDB_Document::factory($model, $value);
        }
      }
      return $this->_related_objects[$name];
    }
  ...
  public function __set($name, $value)
  {
    ...
    // Automatically save references to other MongoDB_Document objects
    if(array_key_exists($name, $this->_references))
    {
      if( ! $value instanceof MongoDB_Document)
      {
        throw new Exception('Cannot set reference to object that is not a MongoDB_Document');
      }
  ...
  protected function _update_references($safe = TRUE)
  {
    ...
    {
      if(isset($this->_related_objects[$name]) && $this->_related_objects[$name] instanceof MongoDB_Document)
      {
   ...
   *
   * @param   string  $action  The type of save action, one of MongoDB_Document::SAVE_*
   */
