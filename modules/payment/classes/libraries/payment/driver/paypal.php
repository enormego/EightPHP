...
class Payment_Driver_Paypal implements Payment_Driver {
	...
	*/
	public function __construct($config) {
		...
		Eight::log('debug', 'PayPal Payment Driver Initialized');
	}
	...
	*/
	...
	public function process() {
		...
		if (in_array(FALSE, $this->required_fields))
		{
			...
			throw new Eight_Exception('payment.required', implode(', ', $fields));
		}
		...
		if (empty($this->do_express_checkout_fields['PAYERID'])) {
			throw new Eight_Exception('payment.required', 'PAYERID');
		}
	...
	*/
	protected function set_express_checkout() {
		...
		if (strtoupper($response_array['ACK']) == 'SUCCESS') {
			...
			throw new Eight_User_Exception('SetExpressCheckout ERROR', Eight::debug($response_array));

			Eight::log('error', Eight::debug('SetExpressCheckout response:'.$response_array));
	...
	*/
	protected function get_express_checkout() {
		...
		{
			throw new Eight_User_Exception('GetExpressCheckout ERROR', Eight::debug($this->get_express_checkout_response));

			Eight::log('error', Eight::debug('GetExpressCheckout response:'.$response));
			url::redirect($this->api_connection_fields['ERRORURL']);
		}
	}
	...
	*/
	protected function do_express_checkout_payment() {
		...
		{
			throw new Eight_User_Exception('DoExpressCheckoutPayment ERROR', Eight::debug($this->nvp_response_array));
