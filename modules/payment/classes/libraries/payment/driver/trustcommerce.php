 ...
 *
 * @version		$Id: trustcommerce.php 244 2010-02-11 17:14:39Z shaun $
 *
 * @package		Modules
 * @subpackage	Payments
 * @author		enormego
...
class Payment_Driver_Trustcommerce implements Payment_Driver {
	...
	public function __construct($config) {
		...
		if (!extension_loaded('tclink')) {
			if (!dl($this->tclink_library)) {
				throw new Eight_Exception('payment.no_dlib', $this->tclink_library);
			}
		}
		
		Eight::log('debug', 'TrustCommerce Payment Driver Initialized');
	}
	...
	public function process() {
		...
		elseif ($result['status'] == 'decline')
			return Eight::lang('payment.error', 'payment_Trustcommerce.decline.'.$result[$result['status'].'type']);
		else
			return Eight::lang('payment.error', Eight::lang('payment_Trustcommerce.'.$result['status'].'.'.$result['error']));
	}
