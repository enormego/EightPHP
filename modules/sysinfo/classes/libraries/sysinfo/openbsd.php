...
class SysInfo_OpenBSD_Core extends SysInfo_BSD {
	protected $cpu_regexp	 = "";
	protected $scsi_regexp1 = "";
	protected $scsi_regexp2 = "";
	protected $cpu_regexp2	= "";
	...
	public function network() {
		$netstat_b = self::execute_program('netstat', '-nbdi | cut -c1-25,44- | grep Link | grep -v \'* \'');
		$netstat_n = self::execute_program('netstat', '-ndi | cut -c1-25,44- | grep Link | grep -v \'* \'');
		$lines_b = split("\n", $netstat_b);
		$lines_n = split("\n", $netstat_n);
		$results = array();
