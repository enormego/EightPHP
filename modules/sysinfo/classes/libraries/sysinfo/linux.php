...
class SysInfo_Linux_Core extends SysInfo {
	protected $inifile = "distros.ini";
	protected $distro = "unknown";
	protected $parser;
	...
	public function __construct() {
		...
		$distro_info = self::execute_program('lsb_release','-a 2> /dev/null', false);	// We have the '2> /dev/null' because Ubuntu gives an error on this command which causes the distro to be unknown
	...
	public function chostname() {
		$result = self::rfts( '/proc/sys/kernel/hostname', 1 );
		if ( $result == "ERROR" ) {
			$result = "N.A.";
		} else {
	...
	public function users() {
		$strResult = 0;
			$strBuf = self::execute_program('who', '-q');
		if( $strBuf != "ERROR" ) {
			$arrWho = split( '=', $strBuf );
					$strResult = $arrWho[1];
		}
	...
	public function cpu_info() {
		...
		if ( $bufr != "ERROR" ) {
			...
			foreach ($sparclist as $name) {
				$buf = self::rfts( '/proc/openprom/' . $name . '/ecache-size',1 , 32, false );
				if( $buf != "ERROR" ) {
		...
		$buf = self::rfts( '/proc/acpi/thermal_zone/THRM/temperature', 1, 4096, false );
		if ( $buf != "ERROR" ) {
	...
	public function pci() {
		...
		if( ! $arrResults = $this->parser->parse_lspci() ) {
			$strBuf = self::rfts( '/proc/pci', 0, 4096, false );
			if( $strBuf != "ERROR" ) {
				$arrBuf = explode( "\n", $strBuf );
	...
	public function ide() {
		$results = array();
		$bufd = self::gdc( '/proc/ide', false );

		foreach( $bufd as $file ) {
			if (preg_match('/^hd/', $file)) {
				$results[$file] = array(); 
				$buf = self::rfts("/proc/ide/" . $file . "/media", 1 );
				if ( $buf != "ERROR" ) {
					$results[$file]['media'] = trim($buf);
					if ($results[$file]['media'] == 'disk') {
						$results[$file]['media'] = 'Hard Disk';
						$buf = self::rfts( "/proc/ide/" . $file . "/capacity", 1, 4096, false);
						if( $buf == "ERROR" ) {
							$buf = self::rfts( "/sys/block/" . $file . "/size", 1, 4096, false);
						}
						if ( $buf != "ERROR" ) {
				...
				$buf = self::rfts( "/proc/ide/" . $file . "/model", 1 );
				if ( $buf != "ERROR" ) {
	...
	public function scsi() {
		...
		$bufr = self::execute_program('lsscsi', '-c', false);
		if( $bufr == "ERROR" ) {
			$bufr = self::rfts( '/proc/scsi/scsi', 0, 4096, false);
		}
		if ( $bufr != "ERROR" ) {
			$bufe = explode("\n", $bufr);
			foreach( $bufe as $buf ) {
	...
	public function usb() {
		...
		$bufr = self::execute_program('lsusb', '', false);
		if( $bufr == "ERROR" ) {
	$bufr = self::rfts( '/proc/bus/usb/devices', 0, 4096, false );
				if ( $bufr != "ERROR" ) {
					$bufe = explode("\n", $bufr);
			foreach( $bufe as $buf ) {
					if (preg_match('/^T/', $buf)) {
