 ...
 *
 * @version		$Id: calendar.php 244 2010-02-11 17:14:39Z shaun $
 *
 * @package		Modules
 * @subpackage	Calendar
 * @author		enormego
 ...
 */
class Calendar_Core extends Calendar_Event_Subject {

	// Start the calendar on Sunday by default
	public static $start_monday = NO;
	 ...
	 * @return  object
	 */
	public static function factory($month = nil, $year = nil) {
	 ...
	 * @return  void
	 */
	public function __construct($month = nil, $year = nil) {
	 ...
	 */
	public function event($name = nil) {
	...
	public function standard($name) {
		switch ($name) {
			...
			case 'prev-next':
				...
				$this->attach($this->event()->condition('current', NO)->add_class('prev-next'));
			break;
			case 'holidays':
				// Base event
				$event = $this->event()->condition('current', YES)->add_class('holiday');
				...
				// Attach Easter
				$holiday = clone $event;
				$this->attach($holiday->condition('easter', YES));

				// Attach Memorial Day
				$holiday = clone $event;
				$this->attach($holiday->condition('month', 5)->condition('day_of_week', 1)->condition('last_occurrence', YES));
			...
			case 'weekends':
				// Weekend events
				$this->attach($this->event()->condition('weekend', YES)->add_class('weekend'));
			break;
		}
	 ...
	 * YES if the day is in the month, NO if it is padding.
	 *
	 * @return  array
	 */
	...
	/**
	 * Convert the calendar to HTML using the eight_calendar view.
	 *
	 * @return  string
	 */
	public function render($view='eight_calendar') {
