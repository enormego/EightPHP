 ...
 *
 * @version		$Id: zip.php 244 2010-02-11 17:14:39Z shaun $
 *
 * @package		Modules
 * @subpackage	Archive
 * @author		enormego
...
class Archive_Driver_Zip_Core extends Archive_Driver {
	...
	public function create($paths, $filename = NO) {
		...
		foreach($paths as $set) {
			...
			$this->add_data($set[0], $set[1], isset($set[2]) ? $set[2] : nil);
		}
	...
	public function add_data($file, $name, $contents = nil) {
		...
		$timestamp = date::unix2dos(($contents === nil) ? filemtime($file) : time());

		// Read the file or use the defined contents
		$data = ($contents === nil) ? file_get_contents($file) : $contents;
