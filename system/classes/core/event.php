...
final class Event {
	...
	public static function add($name, $callback) {
		...
		} elseif(in_array($callback, self::$events[$name], YES)) {
	...
	public static function add_before($name, $existing, $callback) {
		if(empty(self::$events[$name]) OR ($key = array_search($existing, self::$events[$name])) === NO) {
	...
	private static function insert_event($name, $key, $callback) {
		if(in_array($callback, self::$events[$name], YES))
			return NO;
	...
	public static function replace($name, $existing, $callback) {
		if(empty(self::$events[$name]) OR ($key = array_search($existing, self::$events[$name], YES)) === NO)
			return NO;
	 ...
	 *
	 * @param   string  event name
	 * @param   array   specific callback to remove, NO for all callbacks
	 * @return  void
	 */
	public static function clear($name, $callback = NO) {
		if($callback === NO) {
			self::$events[$name] = array();
	 ...
	 * @return  void
	 */
	public static function run($name, & $data = nil) {
