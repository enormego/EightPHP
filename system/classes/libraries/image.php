...
class Image_Core {
	 ...
	 * @return  object
	 */
	public static function factory($image, $config = nil) {
	...
	public function __get($property) {
		if(isset($this->image[$property])) {
			...
			throw new Eight_Exception('core.invalid_property', $property, get_class($this));
		}
	}

	/**
	 * Resize an image to a specific width and height. By default, Eight will
	 ...
	 * @return  object
	 */
	public function resize($width, $height, $master = nil) {
		if(!$this->valid_size('width', $width))
			throw new Eight_Exception('image.invalid_width', $width);

		if(!$this->valid_size('height', $height))
			throw new Eight_Exception('image.invalid_height', $height);

		if(empty($width) and empty($height))
			throw new Eight_Exception('image.invalid_dimensions', __FUNCTION__);

		if($master === nil) {
			// Maintain the aspect ratio by default
			$master = Image::AUTO;
		} elseif(!$this->valid_size('master', $master))
			throw new Eight_Exception('image.invalid_master');
	...
	public function crop($width, $height, $top = 'center', $left = 'center') {
		...
		if(!$this->valid_size('top', $top))
			throw new Eight_Exception('image.invalid_top', $top);

		if(!$this->valid_size('left', $left))
			throw new Eight_Exception('image.invalid_left', $left);
	...
	public function flip($direction) {
		if($direction !== self::HORIZONTAL and $direction !== self::VERTICAL)
			throw new Eight_Exception('image.invalid_flip');
	 ...
	 * @return  object
	 */
	public function save($new_image = NO, $chmod = 0644) {
		...
		if(!is_writable($dir))
			throw new Eight_Exception('image.directory_unwritable', $dir);
 	...
 	{ 
 		...
 		$status = $this->driver->process($this->image, $this->actions, $dir, $file, $render = YES); 
