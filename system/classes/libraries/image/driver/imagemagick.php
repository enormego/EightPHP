...
class Image_Driver_ImageMagick_Core extends Image_Driver {
	...
	public function __construct($config) {
		if(empty($config['directory'])) {
			...
			if(!is_file($path = exec('which convert')))
				throw new Eight_Exception('image.imagemagick.not_found');
		...
		if(!is_file(realpath($config['directory']).'/convert'.$this->ext))
			throw new Eight_Exception('image.imagemagick.not_found', 'convert'.$this->ext);
	...
	protected function properties() {
		return array_slice(getimagesize($this->tmp_image), 0, 2, NO);
	}
