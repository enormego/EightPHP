 ...
 * GD Image Driver.
 *
 * @version		$Id: gd.php 244 2010-02-11 17:14:39Z shaun $
 *
 * @package		System
...
class Image_Driver_GD_Core extends Image_Driver {
	...
	public function __construct() {
		...
		if(!function_exists('gd_info'))
			throw new Eight_Exception('image.gd.requires_v2');

		// Get the GD information
		$info = gd_info();

		// Make sure that the GD2 is installed
		if(strpos($info['GD Version'], '2.') === NO)
			throw new Eight_Exception('image.gd.requires_v2');
	}

	public function process($image, $actions, $dir, $file, $render = NO) {
		...
		if(empty($create) OR!function_exists($create))
			throw new Eight_Exception('image.type_not_allowed', $image['file']);

		// Make sure the image type is supported for saving
		if(empty($save) OR!function_exists($save))
			throw new Eight_Exception('image.type_not_allowed', $dir.$file);
		...
		if($status = $this->execute($actions)) {
			...
			imagealphablending($this->tmp_image, YES);
			imagesavealpha($this->tmp_image, YES);

			switch ($save) {
				case 'imagejpeg':
					// Default the quality to 95
					($quality === nil) and $quality = 95;
				break;
				case 'imagegif':
			...
			if($render === NO) {
				...
				$status = isset($quality) ? $save($this->tmp_image, nil, $quality) : $save($this->tmp_image);
			}
	...
	public function rotate($amount) {
		...
		if($status = imagecopymerge($this->tmp_image, $img, 0, 0, 0, 0, imagesx($this->tmp_image), imagesy($this->tmp_image), 100)) {
			// Prevent the alpha from being lost
			imagealphablending($img, YES);
			imagesavealpha($img, YES);
	...
	public function sharpen($amount) {
		...
		if(!function_exists('imageconvolution'))
			throw new Eight_Exception('image.unsupported_method', __FUNCTION__);
