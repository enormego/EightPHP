...
class dBug {
	...
	public function getVariableName() {
		...
		foreach($arrBacktrace as $key=>$call) {
			if($call['function'] == 'debug' && $call['type'] == '::') {
				$arrFile = $arrBacktrace[$key];
				break;
			}
		}
		...
		if(isset($arrFile)) {
			...
			//find call to Eight::debug()
			preg_match('#Eight::debug\((.*?)\);#isu', $code, $arrMatches);
